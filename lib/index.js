...
// 旧 Sonar Chat Completions（/v1/chat/completions）已废弃，将于 2026-09-27 下线。
// 新接口 POST /v1/agent：model 取 namespaced id "perplexity/sonar"，input 是字符串，
// web_search 是显式 tool；citations/sources 来自 typed output[] 数组里的 search_results item。
// 详见 https://docs.perplexity.ai/docs/agent-api/migrate-from-sonar/how-to
async function searchPerplexity(query, maxResults, apiKey, signal, timeRange) {
  ...
  // time filter：把 dsh-free-search 的统一 timeRange 映射到 Perplexity 的 search_recency_filter
  // （枚举：hour|day|week|month|year）和 last_updated_after_filter（绝对日期 MM/DD/YYYY）
  if (timeRange) {
    const filters = {};
  ...
  const data = await response.json();
  // Agent API 的 typed output[] 同时承载 message item 和 search_results item；
  // 顶层 output_text 是 message content 的快捷字段。
  ...
  // sources: 来自 search_results items；每条 result 自带 url/title/snippet/date
  const sources = [];
  for (const item of output) {
